...
function truncateProviderDetail(value, maxLength = 120) {
  ...
  return clean.length > maxLength ? `${clean.slice(0, maxLength - 1)}ΓÇª` : clean
}
...
// Default secure to false ΓÇö tying it to NODE_ENV=production breaks HTTP deployments
...
if (SESSION_KEY) {
  app.use(cookieSession({
    ...
    // so the cookie Expires is refreshed on every response ΓÇö true rolling behaviour.
    maxAge: 15 * 60 * 1000,
    httpOnly: true,
  ...
  app.use((req, res, next) => {
    if (req.session && req.session.username) {
      // 10-second resolution ΓÇö small enough to be imperceptible, large enough to avoid
 ...
 * Errors are non-fatal ΓÇö a warning is logged and the approve flow continues.
 */
 ...
 * `toPath` must already exist (as a hardlink or copy) ΓÇö it is replaced by the encoded output.
 ...
 * Non-fatal ΓÇö on failure the original hardlink at `toPath` is preserved and a warning is logged.
 */
...
function findExternalSubtitles(fromPath) {
  ...
  // Strategy 3: positional match ΓÇö sort video files and subtitle files, pair by index
 ...
 * e.g. source/video.eng.srt ΓåÆ output/MovieName.S01E01.eng.srt
 */
...
const APPROVED_SERIES_NEGATIVE_CACHE_TTL_MS = 24 * 60 * 60 * 1000; // 24 hours: retry the next day if no image was found
      ...
      debounceTimer = setTimeout(async () => {
        try {
          ...
          if (!prior || !prior.files || Object.keys(prior.files).length === 0) {
            // No prior cache at all ΓÇö fall back to a full scan so we get everything
            ...
            // Persist updated cache ΓÇö must happen before buildIncrementalItems
            if (currentCache) saveScanCacheFn(currentCache);

            // Build the full item list from currentCache (all known files), prioritising
            // new/changed entries first. NOTE: pass currentCache, NOT scanCache ΓÇö the old
...
async function fetchTmdbById(id, apiKey, season, episode, mediaTypeHint = null) {
  ...
  if (mediaTypeHint === 'movie') {
    // User explicitly said this is a movie ΓÇö skip the TV probe
    ...
    // User explicitly said this is a TV series ΓÇö skip the movie probe
...
async function fetchTvdbById(id, creds, season, episode, log, mediaTypeHint = null) {
  ...
  if (mediaTypeHint === 'movie') {
    // User explicitly said this is a TVDB movie ΓÇö use the movies endpoint
...
async function metaLookup(title, apiKey, opts = {}) {
  ...
  function normalizeSearchQuery(s) {
    ...
    out = out.replace(/\([^\)]*\b(?:1080p|720p|2160p|x264|x265|webrip|web-dl|bluray|hdtv|aac|dual audio)\b[^\)]*\)/ig, ' ')
      ...
      const dashSplit = out.split(/\s[-ΓÇôΓÇö]\s/)
  ...
  function extractSeasonNumberFromTitle(t) {
    try {
      ...
      // "Part N" and "Cour N" ΓÇö AniList frequently labels anime cours/arcs this way.
      ...
      if (m && m[1]) return parseInt(m[1], 10)
      // fallback: trailing digits like "Title 2" ΓÇö only treat as season when
...
async function _externalEnrichImpl(canonicalPath, providedKey, opts = {}) {
  ...
  // Compute a parent-folder candidate but do NOT prefer it yet ΓÇö we'll try filename first, then parent if TMDb fails.
      ...
      if (res && res.name) {
        if (res.provider === 'anidb') {
          try {
            ...
            // Movies are not forced to S00 ΓÇö they get isMovie=true instead.
            if (animeType && episodeSeason !== 0) {
              ...
              if (['tv special', 'ova', 'ona', 'music video', 'special'].some(t => animeTypeLower.includes(t))) {
                console.log(`[Server] AniDB animeType "${animeType}" ΓåÆ overriding season to 0`);
                ...
                if (typeof guess.isMovie !== 'boolean') {
                  guess.isMovie = true;
                  console.log(`[Server] AniDB animeType "Movie" ΓåÆ setting isMovie=true`);
  ...
  if (episodeLabelMatch) {
    ...
    if (prefix.length < maxLen - 5) { // -5 for " - " + ellipsis
      ...
      if (remainingLen > 10) { // Only if we have meaningful space for episode title
        const truncatedEpTitle = episodeTitle.slice(0, remainingLen - 1) + 'ΓÇª';
  ...
  try {
    ...
    if (parsed) {
      ...
      try {
        ...
        try { parsedRendered = parsedRendered.replace(/\s*\(\s*\)\s*/g, '').replace(/\s*[-ΓÇôΓÇö]\s*$/g, '').replace(/\s{2,}/g, ' ').trim(); } catch (e) {}
...
// ΓöÇΓöÇΓöÇ Server-Sent Events (SSE) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
app.post('/api/enrich', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // Capture the previous enrichment first so we can restore it if the rescan fails ΓÇö this ensures the
  ...
  try { if (db) db.setKV('enrichCache', enrichCache); else writeJson(enrichStoreFile, enrichCache); } catch (e) {}
    ...
    const enrichHandlerPromise = (async () => {
      ...
      try {
        if (data && data.title) {
          ...
          // No title found ΓÇö avoid persisting an empty entry on a force rescan.
      ...
      // enrichment ΓÇö the DB was never purged (persist: false above) so it already holds the
    ...
    if (handlerResult === '__timeout__') {
      // Enrichment is still running in background ΓÇö respond immediately with whatever
...
// GET /api/enrich/active ΓÇö returns paths currently being enriched by POST /api/enrich.
...
// Standalone plan generator ΓÇö used by the preview route and the server-side approve job runner.
...
function generatePlanForItem(it, { username, effectiveOutput, applyFilenameAsTitle, template }) {
  ...
  function cleanTitleForRender(baseTitle, epLabel, epTitle) {
    try {
      ...
      if (!cleaned) return '';
      cleaned = cleaned.replace(/\s*[-ΓÇôΓÇö:]+\s*S\d{1,2}E\d{1,3}(?:\s*[-ΓÇôΓÇö:]+\s*.*)?$/i, '');
      cleaned = cleaned.replace(/\s*[-ΓÇôΓÇö:]+\s*E\d{1,3}(?:\s*[-ΓÇôΓÇö:]+\s*.*)?$/i, '');
      cleaned = cleaned.replace(/\s*[-ΓÇôΓÇö:]+\s*Episode\s+\d+.*$/i, '');
      return cleaned.trim();
    } catch (e) {
  ...
  if (applyFilenameAsTitle && filenameBase) {
    ...
    try {
      ...
      if (shouldStripSeason) {
        const parts = providerName.split(/\s[-ΓÇôΓÇö:]\s/);
...
function ensureRenderedNameHasYear(name, year) {
  try {
    ...
    const episodeMarkerPattern = /[\s\-ΓÇôΓÇö]*(?:S\d{1,2}E\d{1,3}|E\d{1,3})\b/i;
...
function stripEpisodeArtifactsForFolder(name) {
  try {
    ...
    if (!out) return out;
    out = out.replace(/\s*[-ΓÇôΓÇö:]+\s*S\d{1,2}E\d{1,3}(?:\s*[-ΓÇôΓÇö:]+\s*.*)?$/i, '');
    out = out.replace(/\s*[-ΓÇôΓÇö:]+\s*E\d{1,3}(?:\s*[-ΓÇôΓÇö:]+\s*.*)?$/i, '');
    out = out.replace(/\s*[-ΓÇôΓÇö:]+\s*Episode\s+\d+.*$/i, '');
    return out.trim();
  } catch (e) {
    return String(name || '').trim();
  }
...
function stripSeasonNumberSuffix(name) {
  try {
    ...
    s = s.replace(/\s*[\-ΓÇôΓÇö:\/\(]?\s*(?:Season|Series|Part)\s*(?:#\s*)?(?:\b(?:[0-9]{1,2}|[IVXLC]+|first|second|third|fourth|fifth|sixth|seventh|eighth|ninth|tenth|[0-9]{1,2}(?:st|nd|rd|th))\b)\s*\)?$/i, '').trim();
...
function looksLikeEpisodeTitleCandidate(title, episodeTitle) {
  ...
  if (episodeTitle) {
    ...
    if (epTrim) {
      try {
        const suffixRe = new RegExp(`(?:[\-ΓÇôΓÇö:\s]+)?${escapeRegExp(epTrim)}$`, 'i');
  ...
  try {
    ...
    if (epLabel) {
      ...
      if (lbl) {
        ...
        s = s.replace(new RegExp('[\\s\\-ΓÇôΓÇö:]*' + escapeRegExp(lbl) + '(?=[\\s\\-ΓÇôΓÇö:]|$)', 'gi'), '').trim();
      }
    }
    // Also strip any remaining SxxExx patterns anywhere in the string
    s = s.replace(/[\s\-ΓÇôΓÇö:]*S\d{1,2}E\d{1,3}(?=[\\s\-ΓÇôΓÇö:]|$)/gi, '').trim();
    s = s.replace(/^\s*S\d{1,2}[\s_\-:\.]*[EPp]?(\d{1,3})?(?:\.\d+)?[\s_\-:\.]*/i, '').trim();
    if (epTitle) {
      const et = String(epTitle).trim();
      if (et) s = s.replace(new RegExp('[\-ΓÇôΓÇö:\\s]*' + escapeRegExp(et) + '$', 'i'), '').trim();
    }
    s = s.replace(/^[\-ΓÇôΓÇö:\s]+|[\-ΓÇôΓÇö:\s]+$/g, '').trim();
  } catch (e) { /* best-effort */ }
...
async function fetchAniListSeriesArtworkWithCandidates(candidates) {
  try {
    ...
    //   "Hanamonogatari Suruga Devil" ΓåÆ "Hanamonogatari Suruga" ΓåÆ "Hanamonogatari"
    //   "Title: Subtitle"             ΓåÆ "Title"
    ...
    for (const candidate of queue) {
      ...
      const colonStripped = base.replace(/\s*[:ΓÇôΓÇö]\s*.+$/, '').trim();
    ...
    // Step 3: AniList synonym expansion ΓÇö query AniList with the best candidate we have
...
// ΓöÇΓöÇΓöÇ AniDB Titles Database ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
//   2. Parse every <title> element ΓåÆ Map<normalizedTitle, aid>
//   3. Resolve AID offline ΓÇö zero HTTP API rate-limit cost for title lookup
...
let _anidbTitlesMap = null;        // plain object { title -> aid } ΓÇö built once
...
function _anidbComparableName(name) {
  const REMOVE  = new Set([...'"\' !`?']);
  const SPACERS = new Set([...'/,.:;\\(){}[]+-_=\u2013*']);
  let result = String(name || '').toLowerCase().normalize('NFC');
  let sb = '';
  for (const ch of result) {
    const cp = ch.codePointAt(0);
    if (cp >= 0x2B0 && cp <= 0x0333) continue;  // skip modifiers / diacritics
    else if (REMOVE.has(ch))  continue;           // drop punctuation
    else if (SPACERS.has(ch)) sb += ' ';          // spacers ΓåÆ space
    else if (ch === '&')      sb += ' and ';
    else                      sb += ch;
  }
  result = sb;
  result = result.replace(/, the/g, '').replace(/the /g, ' ').replace(/ the /g, ' ');
  let prev;
  do { prev = result; result = result.replace(/  +/g, ' '); } while (result !== prev);
  return result.trim();
}

function _buildAnidbTitlesMap(xml) {
  ...
  while ((m = animeRe.exec(xml)) !== null) {
    ...
    let t;
    while ((t = titleRe.exec(m[2])) !== null) {
      const raw = t[1].trim();
      if (!raw) continue;
      if (!map[raw]) map[raw] = aid;
      const comp = _anidbComparableName(raw);
      if (comp && !map[comp]) map[comp] = aid;
    }
  }
  return map;
}

async function _downloadAnidbTitlesXml() {
  return new Promise((resolve, reject) => {
    require('https').get(ANIDB_TITLES_URL, { headers: { 'User-Agent': 'mediabrowser/1' } }, (res) => {
      const chunks = [];
      res.on('data', (c) => chunks.push(c));
      res.on('end',  () => {
        require('zlib').gunzip(Buffer.concat(chunks), (err, buf) => {
          if (err) reject(err); else resolve(buf.toString('utf8'));
        });
      });
      res.on('error', reject);
    }).on('error', reject).setTimeout(90000, function () { this.destroy(new Error('titles download timeout')); });
  });
}

async function _doLoadAnidbTitlesDb() {
  try {
    if (fs.existsSync(ANIDB_TITLES_FILE)) {
...
function loadAnidbTitlesDb() {
  if (_anidbTitlesMap) return Promise.resolve();
  if (!_anidbTitlesLoadPromise) {
    _anidbTitlesLoadPromise = _doLoadAnidbTitlesDb().finally(() => { _anidbTitlesLoadPromise = null; });
  }
  return _anidbTitlesLoadPromise;
}

function anidbLookupTitle(seriesName) {
  if (!_anidbTitlesMap) return null;
  const name = String(seriesName || '').trim();
  return _anidbTitlesMap[name] || _anidbTitlesMap[_anidbComparableName(name)] || null;
}

// Per-session per-AID anime cache ΓÇö avoids duplicate HTTP calls within one server run
...
async function findAniDbAidByTitle(seriesName) {
  // Use the offline titles database ΓÇö no API calls, no rate-limit cost.
...
async function fetchAniDbSeriesArtwork(seriesName, outputKey, username, titleCandidates) {
  ...
  //   4. Parse <picture> element ΓåÆ https://cdn.anidb.net/images/main/{picture}
  try {
    ...
    // 3. No AID found ΓÇö fall back to HTTP title search for each candidate title.
    ...
    // 3b. Offline DB missed for all candidates ΓÇö fall back to aname= HTTP title search.
    //     Only try ASCII/latin-script titles; AniDB's aname= endpoint is unreliable for
    //     CJK/native-script characters and wastes time on 500ms-per-call timeouts.
    for (const candidate of candidates) {
      if (!candidate) continue;
      // Skip non-ASCII titles ΓÇö AniDB's aname= search doesn't handle them reliably
    ...
    //    native-script title searches ΓÇö a single AniList call resolves the AID directly.
    ...
    //    5a. Try ALL synonyms (including native/CJK) against the offline titles DB ΓÇö free,
    ...
    if (novelSynonymsAll.length > 0) {
      ...
      // 5a. Offline titles DB lookup for each synonym ΓÇö handles CJK/native titles too
...
// ΓöÇΓöÇΓöÇ Auto-Rescan Background Worker ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Background Job Queue ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// GET /api/jobs ΓÇö list recent/active jobs (useful for reconnecting clients)
...
// GET /api/jobs/:id ΓÇö poll a specific job
...
// POST /api/jobs/approve ΓÇö run preview + apply entirely server-side
...
// POST /api/jobs/backfill-subtitles ΓÇö for all approved items, extract subtitles if the .srt is missing
...
// POST /api/jobs/bulk-rescan ΓÇö enrich a list of paths server-side with rate limiting
...
app.get('/api/logs/recent', requireAuth, requireAdmin, (req, res) => {
  try {
    ...
    // side ΓÇö leaving no visible lines unless we read back far enough to find older ones.
