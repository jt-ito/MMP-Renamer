...
const fs = require('fs');
let crypto = null;
let cryptoMd4Available = false;

try {
  crypto = require('crypto');
  try {
    const test = crypto.createHash('md4');
    test.update(Buffer.alloc(0));
    test.digest();
    cryptoMd4Available = true;
  } catch (err) {
    cryptoMd4Available = false;
    crypto = null;
  }
} catch (err) {
  crypto = null;
...
const { md4: md4Fallback } = require('./md4');
...
function computeMD4(data) {
  if (cryptoMd4Available && crypto) {
    const hash = crypto.createHash('md4');
    hash.update(data);
    return hash.digest();
  }
  return md4Fallback(data);
}
