...
function defaultCanonicalize(p) { return path.resolve(p).replace(REGEX_BACKSLASH, '/') }
  ...
  for (const p of Object.keys(priorFiles || {})) {
    try {
      ...
      if (st && st.mtimeMs != null) {
        ...
        currentFiles[p] = { mtime: st.mtimeMs, size: st.size, id: newId };
        if (!priorEntry || priorEntry.mtime !== st.mtimeMs || priorEntry.size !== st.size) {
          toProcess.push({ id: newId, canonicalPath: p, scannedAt: Date.now() });
        }
      }
    } catch (e) {
        ...
        if (extRe.test(e.name)) {
          ...
          if (!currentFiles[c]) {
            try {
              ...
              currentFiles[c] = { mtime: st.mtimeMs, size: st.size, id };
...
function buildIncrementalItems(currentCache, toProcess, makeId) {
  ...
  // Add new/modified files first (from toProcess)
  ...
  // Then add all previously scanned files from cache (filtering happens in server.js)
