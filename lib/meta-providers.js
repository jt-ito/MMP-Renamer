...
async function lookupMetadataWithAniDB(filePath, title, opts = {}, forceHash = false) {
  ...
  if (opts.anidb_username && opts.anidb_password && filePath && fs.existsSync(filePath)) {
    ...
    try {
      ...
      // Get AniDB UDP client
      const anidbClient = getAniDBUDPClient(opts.anidb_username, opts.anidb_password, 'mmprename', 1);
      ...
      if (fileInfo) {
        ...
        result.name = fileInfo.animeEnglishName || fileInfo.animeRomajiName || fileInfo.animeKanjiName || title;
        result.episodeTitle = fileInfo.episodeEnglishName || fileInfo.episodeRomajiName || fileInfo.episodeKanjiName || null;
