  ...
  if (opts.anidb_username && opts.anidb_password && filePath && fs.existsSync(filePath)) {
    ...
    try {
      ...
      if (fileInfo) {
        ...
        const englishMatchesInput = englishEntry && normalizedInput && normalizeTitleKey(englishEntry.value) === normalizedInput;

        if (englishMatchesInput) {
        ...
        const episodeEnglish = pickTitle(fileInfo.episodeEnglishName);
        const episodeRomaji = pickTitle(fileInfo.episodeRomajiName);
        const episodeKanji = pickTitle(fileInfo.episodeKanjiName);
        ...
        result.episodeNumber = fileInfo.episodeNumber ? parseEpisodeNumber(fileInfo.episodeNumber) : (opts.episode != null ? opts.episode : null);
        result.episodeNumberRaw = fileInfo.episodeNumber || null;
