 ...
 * Prioritizes "Season X" over "Part X" for titles like "Season 2 Part 2"
...
function extractPartCourSeason(titleList) {
  ...
  // Priority 1: Explicit Season patterns (highest priority)
  const seasonPatterns = [
    /Season\s+(\d+)/i,
    /S(\d+)\s+/i  // Match S2 Part 2, etc.
  ];
  
  // Priority 2: Part/Cour patterns (use only if no Season found)
  const partCourPatterns = [
    /Part\s+(\d+)/i,
    /(\d+)(?:st|nd|rd|th)\s+Part/i,
    /Cour\s+(\d+)/i,
    /Part\s+(II|III|IV|V|VI|VII|VIII|IX|X)/i,  // Roman numerals
    /(\d+)\s+Cour/i
  ];
  ...
  // First pass: Check for explicit Season patterns
    ...
    for (const pattern of seasonPatterns) {
      ...
      if (match) {
        ...
        if (num > 0 && num < 20) {  // Sanity check
          console.log(`[MetaProviders] Detected explicit season from pattern: "${title}" -> Season ${num}`);
          return num;
        }
      }
    }
  ...
  // Second pass: Check for Part/Cour patterns only if no Season found
  for (const title of titleList) {
    if (!title || typeof title !== 'string') continue;
    ...
    for (const pattern of partCourPatterns) {
      ...
      if (match) {
        ...
        if (romanToNumber[captured]) {
          console.log(`[MetaProviders] Detected Part/Cour from pattern: "${title}" -> Season ${romanToNumber[captured]}`);
          return romanToNumber[captured];
        }
        // Otherwise parse as number
        const num = parseInt(captured, 10);
        if (num > 0 && num < 20) {  // Sanity check
          console.log(`[MetaProviders] Detected Part/Cour from pattern: "${title}" -> Season ${num}`);
          return num;
        }
      }
    }
...
async function lookupMetadataWithAniDB(filePath, title, opts = {}, forceHash = false) {
  ...
  if (!opts.season) {
    // Check all available title variations for Part/Cour patterns
    const allTitles = [
      englishTitle,
      romajiTitle,
      ...shortNames,
    ...
    detectedSeason = extractPartCourSeason(allTitles);
    if (detectedSeason) {
      console.log(`[MetaProviders] Detected Part/Cour season ${detectedSeason} from AniDB titles`);
    }
  }
  
  result.seasonNumber = (opts && typeof opts.season !== 'undefined' && opts.season !== null) ? opts.season : detectedSeason;
        result.raw = fileInfo;
