...
async function lookupMetadataWithAniDB(filePath, title, opts = {}) {
  ...
  if (opts.anidb_username && opts.anidb_password && filePath && fs.existsSync(filePath)) {
    // Check file size - skip AniDB for files larger than 500MB to avoid timeout
    const MAX_ANIDB_FILE_SIZE = 500 * 1024 * 1024; // 500MB
    try {
      const stats = fs.statSync(filePath);
      if (stats.size > MAX_ANIDB_FILE_SIZE) {
        console.log(`[MetaProviders] File too large for AniDB hash lookup (${Math.round(stats.size/1024/1024)}MB > 500MB), skipping`);
        return null;
      }
    } catch (sizeErr) {
      console.error('[MetaProviders] Failed to check file size:', sizeErr.message);
      return null;
    }
