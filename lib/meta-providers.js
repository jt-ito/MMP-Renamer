...
// AniDB concurrency lock: serializes all lookupMetadataWithAniDB calls so only
// one UDP session sequence runs at a time, preventing AUTH command pile-ups when
// multiple items are rescanned simultaneously.
let _anidbLockChain = Promise.resolve();
function _acquireAnidbLock(fn) {
  const next = _anidbLockChain.then(() => fn()).catch(() => fn());
  _anidbLockChain = next.then(() => {}, () => {});
  return next;
}
...
async function lookupMetadataWithAniDB(filePath, title, opts = {}, forceHash = false) {
      ...
      if (episodeInfo && episodeInfo.aid) {
        ...
        if (animeResponse) {
          ...
          if (animeResponse._allFields && Array.isArray(animeResponse._allFields)) {
            ...
            // Scan fields 4-8 for title patterns (field 3 is categories/tags)
            const titleFields = fields.slice(4, 9).map((f, idx) => ({
              index: idx + 4,
              value: f,
            ...
            // Identify English (Latin only, shorter/cleaner), Romaji (Latin with punctuation), Kanji (Japanese)
            const kanjiCandidate = titleFields.find(f => f.hasJapanese && !f.hasLatin);
            const latinFields = titleFields.filter(f => f.hasLatin && !f.hasJapanese);
            
            // English is typically shorter and cleaner than romaji
            const englishCandidate = latinFields.find(f => !f.value.includes(',') && f.length < 30);
            const romajiCandidate = latinFields.find(f => f !== englishCandidate && !f.value.includes(','));
            
            console.log('[MetaProviders] Detected candidates:', {
              english: englishCandidate?.value?.slice(0, 30),
              romaji: romajiCandidate?.value?.slice(0, 30),
              kanji: kanjiCandidate?.value?.slice(0, 30)
            });
            
            // Override with detected fields if parsing seems off
            if (englishCandidate && (!englishTitle || englishTitle === kanjiTitle || englishTitle === romajiTitle)) {
              englishTitle = englishCandidate.value;
              console.log('[MetaProviders] Corrected english from field', englishCandidate.index, ':', englishTitle.slice(0, 30));
            }
            if (romajiCandidate && (!romajiTitle || romajiTitle.includes('Restricted') || romajiTitle.includes(','))) {
              romajiTitle = romajiCandidate.value;
              console.log('[MetaProviders] Corrected romaji from field', romajiCandidate.index, ':', romajiTitle.slice(0, 30));
            }
            if (kanjiCandidate && (!kanjiTitle || !hasJapaneseChars(kanjiTitle))) {
              kanjiTitle = kanjiCandidate.value;
              console.log('[MetaProviders] Corrected kanji from field', kanjiCandidate.index, ':', kanjiTitle.slice(0, 30));
            }
          }
          ...
          const parsedEpNumRaw = parseEpisodeNumber(episodeInfo.epno);
          // Adjust for animeType: TV Special / OVA / ONA → season 0; Movie → isMovie
          const epNumHandling = adjustEpisodeForAnimeType(parsedEpNumRaw, animeType);
          const parsedEpNum = epNumHandling.episodeNumber;
          ...
          // If animeType is a special or movie type, pin to season 0
          if (epNumHandling.isSpecialType || epNumHandling.isMovieType) {
            result.seasonNumber = 0;
          } else {
            result.seasonNumber = opts.season || 1;
          }
          if (epNumHandling.isMovieType) result.isMovie = true;
          result.raw = rawForMapping;
          result.source = 'AniDB Manual Episode ID';
  ...
  const rawEpnoEd2k = episodeInfo?.epno || fileInfo.episodeNumber || null;
  const parsedEpnoEd2kRaw = rawEpnoEd2k ? parseEpisodeNumber(rawEpnoEd2k) : (opts.episode != null ? opts.episode : null);
  // Adjust for animeType: TV Special / OVA / ONA → season 0; Movie → isMovie
  const ed2kEpHandling = adjustEpisodeForAnimeType(parsedEpnoEd2kRaw, animeType);
  result.episodeNumber = ed2kEpHandling.episodeNumber;
  result.episodeNumberRaw = rawEpnoEd2k;
  if (ed2kEpHandling.isMovieType) result.isMovie = true;
  ...
  // If animeType indicates special/movie, always pin to season 0 (overrides Part/Cour detection)
  if (ed2kEpHandling.isSpecialType || ed2kEpHandling.isMovieType) {
    result.seasonNumber = 0;
  } else {
...
// AniDB anime types that map to Season 0 (specials) vs movies
const ANIDB_SPECIAL_ANIME_TYPES = new Set(['tv special', 'ova', 'ona', 'music video', 'special']);
const ANIDB_MOVIE_ANIME_TYPES = new Set(['movie']);

/**
 * Given a parsed episode number and AniDB animeType, determine if the
 * season/episode should be adjusted for special or movie content.
 *
 * - TV Special / OVA / ONA / Music Video  → season 0, episode N  (isSpecialType)
 * - Movie                                 → isMovieType = true, season unchanged
 * - Already at season 0 ("0.X" string)   → flagged as special, no number change
 *
 * @param {number|string|null} parsedEpNum  Result of parseEpisodeNumber()
 * @param {string|null}        animeType    Raw animeType string from AniDB
 * @returns {{ episodeNumber: number|string|null, isSpecialType: boolean, isMovieType: boolean }}
 */
function adjustEpisodeForAnimeType(parsedEpNum, animeType) {
  if (!animeType) {
    return { episodeNumber: parsedEpNum, isSpecialType: false, isMovieType: false };
  }

  const typeNorm = String(animeType).toLowerCase().trim();
  const isMovieType = ANIDB_MOVIE_ANIME_TYPES.has(typeNorm);
  const isSpecialType = ANIDB_SPECIAL_ANIME_TYPES.has(typeNorm) || typeNorm.includes('special');

  if (!isSpecialType && !isMovieType) {
    return { episodeNumber: parsedEpNum, isSpecialType: false, isMovieType: false };
  }

  // Already at season 0 — just surface the flags
  if (typeof parsedEpNum === 'string' && parsedEpNum.startsWith('0.')) {
    return { episodeNumber: parsedEpNum, isSpecialType: isSpecialType || isMovieType, isMovieType };
  }

  // Specials: convert regular integer episode to "0.N" season-0 format
  if (isSpecialType && parsedEpNum != null) {
    const numVal = Number(parsedEpNum);
    if (!isNaN(numVal)) {
      console.log(`[MetaProviders] animeType "${animeType}" → mapping episode ${numVal} to season 0 (0.${numVal})`);
      return { episodeNumber: `0.${numVal}`, isSpecialType: true, isMovieType: false };
    }
  }

  // Movies: mark as movie but don't alter episode numbering
  return { episodeNumber: parsedEpNum, isSpecialType: false, isMovieType };
}
...
// The original implementation (now called with the lock wrapper below)
const _lookupMetadataWithAniDBImpl = lookupMetadataWithAniDB;

/**
 * Serialized wrapper: ensures only one AniDB lookup runs at a time.
 * Concurrent calls queue behind the active lookup rather than all firing
 * AUTH + command sequences simultaneously (which causes the "10x back-to-back" issue).
 */
async function lookupMetadataWithAniDBSerial(filePath, title, opts, forceHash) {
  return _acquireAnidbLock(() => _lookupMetadataWithAniDBImpl(filePath, title, opts, forceHash));
}

module.exports = {
  lookupMetadataWithAniDB: lookupMetadataWithAniDBSerial,
  getAniDBCredentials,
  parseEpisodeNumber,
  adjustEpisodeForAnimeType
};
