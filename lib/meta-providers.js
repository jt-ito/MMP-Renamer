...
async function lookupMetadataWithAniDB(filePath, title, opts = {}, forceHash = false) {
  ...
  if (opts.anidb_username && opts.anidb_password && filePath && fs.existsSync(filePath)) {
    ...
    try {
      ...
      // If forceHash=true and no cached hash, compute it now (blocking)
      if (!ed2kHash && forceHash) {
        console.log('[MetaProviders] forceHash=true - computing ED2K hash inline:', filePath, `(${fileSizeMB}MB)`);
        try {
