...
class AniDBClient {
  ...
  _parseHttpAnimeResponse(xmlData) {
    ...
    // Extract picture - AniDB uses <picture> tag with filename
    const picture = extractTag('picture');
    console.log('[AniDB] Parsed anime - picture:', picture);
    
    // Log a sample of the XML to debug
    if (!picture && xmlData.length > 0) {
      console.log('[AniDB] No picture found. XML sample (first 500 chars):', xmlData.slice(0, 500));
      console.log('[AniDB] Checking for picture tag variations...');
      // Check for common variations
      const picMatch = xmlData.match(/<picture[^>]*>([^<]*)<\/picture>/i);
      const picnameMatch = xmlData.match(/<picname[^>]*>([^<]*)<\/picname>/i);
      const imageMatch = xmlData.match(/<image[^>]*>([^<]*)<\/image>/i);
      console.log('[AniDB] picture tag match:', picMatch ? picMatch[1] : null);
      console.log('[AniDB] picname tag match:', picnameMatch ? picnameMatch[1] : null);
      console.log('[AniDB] image tag match:', imageMatch ? imageMatch[1] : null);
    }
    ...
    return {
      ...
      picture: picture,  // Add picture field
      raw: xmlData
    };
  }
}
