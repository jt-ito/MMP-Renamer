...
class AniDBClient {
      ...
      try {
        ...
        if (callback) {
          ...
          if (code.startsWith('2')) {
            // Success codes (2xx)
            // FILE responses are on a single line: "{tag} {code} {data}"
            // Extract everything after the code
            const dataMatch = firstLine.match(/^\d+\s+\d+\s+(.*)$/);
            const dataToPass = dataMatch ? dataMatch[1] : lines.slice(1).join('\n');
  ...
  _parseFileResponse(data) {
    ...
    if (fields.length > 0) {
      console.log('[AniDB] _parseFileResponse field[0] (fid):', fields[0]);
      console.log('[AniDB] _parseFileResponse field[11] (animeTitle):', fields[11]);
    }
