...
class AniDBClient {
  ...
  _parseHttpAnimeResponse(xmlData) {
    ...
    // Extract picture - try multiple patterns
    let picture = extractTag('picture');
    
    // If not found, try more aggressive patterns
    if (!picture) {
      const patterns = [
        /<picture[^>]*>([^<]+)<\/picture>/i,
        /<picname[^>]*>([^<]+)<\/picname>/i,
        /<image[^>]*>([^<]+)<\/image>/i,
        // Sometimes it's in a URL attribute
        /picture="([^"]+)"/i,
        /picname="([^"]+)"/i
      ];
      ...
      for (const pattern of patterns) {
        ...
        if (match && match[1]) {
          picture = match[1];
          console.log('[AniDB] Found picture with pattern:', pattern.toString());
          break;
        }
      }
    }
    
    // Log for debugging - but only first 800 chars to avoid spam
    if (!picture) {
      console.log('[AniDB] No picture tag found. XML length:', xmlData.length);
      console.log('[AniDB] XML sample:', xmlData.slice(0, 800));
    } else {
      console.log('[AniDB] Parsed picture:', picture);
    }
  ...
  /**
   * Scrape picture filename from AniDB web page as fallback
   */
  async scrapePictureFromWeb(aid) {
    try {
      await this._waitForHttpRateLimit();
      
      return new Promise((resolve, reject) => {
        const req = https.get(`https://anidb.net/anime/${aid}`, (res) => {
          ...
          res.on('end', () => {
            try {
              ...
              // Look for picture in various formats
              const patterns = [
                /cdn\.anidb\.net\/images\/main\/([^"'\s]+\.(?:jpg|png|gif))/i,
                /cdn-eu\.anidb\.net\/images\/main\/([^"'\s]+\.(?:jpg|png|gif))/i,
                /images\/main\/([^"'\s]+\.(?:jpg|png|gif))/i,
                /picture[^>]*>\s*([^<]+\.(?:jpg|png|gif))/i
              ];
              
              for (const pattern of patterns) {
                const match = html.match(pattern);
                if (match && match[1]) {
                  console.log('[AniDB] Scraped picture from web:', match[1]);
                  resolve(match[1]);
                  return;
                }
              }
              ...
              console.error('[AniDB] Scrape parse error:', e.message);
              resolve(null);
            }
          });
        });
        ...
        req.on('error', (err) => {
          console.error('[AniDB] Scrape request error:', err.message);
          resolve(null);
        });
      ...
      console.error('[AniDB] Scrape error:', e.message);
      return null;
    }
  }
}
