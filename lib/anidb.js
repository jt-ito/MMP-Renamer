...
const ANIDB_CLIENT_NAME = 'mmprename'; // registered client name for this application
...
class AniDBClient {
  ...
  async _sendUdpCommand(command, params = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      }, 10000); // 10 second timeout — keeps UDP from blocking the pipeline for too long
  ...
  async getAnimeInfo(aid) {
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log('[AniDB HTTP] GET', url);

      const req = http.get(url, (res) => {
        console.log('[AniDB HTTP] response status=' + res.statusCode + ' after ' + (Date.now() - reqStart) + 'ms url=' + url.slice(0, 120));
        const chunks = [];
        let totalLength = 0;
        ...
        res.on('end', () => {
          try {
            ...
            console.log('[AniDB HTTP] decoded body length=' + String(data || '').length + ' first300=' + String(data || '').slice(0, 300));
      ...
      req.on('error', (err) => { console.log('[AniDB HTTP] req error after ' + (Date.now() - reqStart) + 'ms: ' + err.message); done(reject, err); });
      req.once('socket', (sock) => {
        sock.once('connect', () => console.log('[AniDB HTTP] TCP connected after ' + (Date.now() - reqStart) + 'ms'));
      });

      // Absolute wall-clock timeout — fires regardless of socket state
      const timer = setTimeout(() => {
        console.log('[AniDB HTTP] absolute timeout after ' + (Date.now() - reqStart) + 'ms, destroying req');
        req.destroy();
        done(reject, new Error('AniDB HTTP request timeout'));
      }, 20000);
      // Also clear the timer if the promise settles normally
  ...
  _parseHttpAnimeResponse(xmlData) {
    console.log('[AniDB HTTP] _parseHttpAnimeResponse raw body (first 800):', String(xmlData || '').slice(0, 800));
    // AniDB returns <error>msg</error> OR <error code="NNN">msg</error>
    if (/<error[\s>]/i.test(xmlData)) {
      const errorMatch = xmlData.match(/<error[^>]*>([^<]+)<\/error>/);
      const errMsg = errorMatch ? errorMatch[1] : 'Unknown error';
      console.log('[AniDB HTTP] API returned error element:', errMsg, '| raw (first 400):', String(xmlData || '').slice(0, 400));
      const err = new Error(`AniDB HTTP error: ${errMsg} | raw=${String(xmlData || '').slice(0, 300)}`);
      err.rawXml = String(xmlData || '').slice(0, 1000);
      throw err;
    }
