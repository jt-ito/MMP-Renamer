...
const REGEX_TAG_CODE_DATA = /^(\d+)\s+(\d{3})\s*(.*)$/
const REGEX_TAG_ONLY = /^(\d+)\s+/
const REGEX_CODE_DATA = /(\d{3})\s+(.*?)$/
const REGEX_SESSION = /^(\S+)/
const REGEX_PIPE_SPLIT = /\|/
const REGEX_NEWLINE = /\n/
...
class AniDBUDPClient {
  ...
  const firstLine = lines[0]
      ...
      let match = firstLine.match(REGEX_TAG_CODE_DATA)
      
      // If that fails, try to find a 3-digit code anywhere in the line after the tag
      if (!match) {
        const tagMatch = firstLine.match(REGEX_TAG_ONLY)
        if (tagMatch) {
          const tag = tagMatch[1]
          const afterTag = firstLine.substring(tagMatch[0].length)
          const codeMatch = afterTag.match(REGEX_CODE_DATA)
          if (codeMatch) {
  ...
  async _sendCommand(command, params = {}, isFileCommand = false) {
    ...
    // Use global rate limiter (handles coordination with HTTP and other UDP requests)
    const requestType = isFileCommand ? 'UDP FILE' : 'UDP';
    await waitForRateLimit(requestType);
   ...
   */
  async loginToAniDB() {
    // Always initialize socket first
    this._initSocket();
    ...
    if (response.code === RESPONSE_CODES.LOGIN_ACCEPTED || 
      ...
      const sessionMatch = response.data.match(REGEX_SESSION)
      if (sessionMatch) {
   ...
   */
  async lookupFile(ed2kHash, fileSize, _retried = false) {
    // Ensure socket is initialized before any operations
    this._initSocket();
    
    // Ensure we're logged in (this will also initialize socket if needed)
    if (!this.sessionKey || !this.loggedIn || Date.now() >= this.sessionExpiry) {
      console.log('[AniDB UDP] Session invalid or expired, logging in...');
  ...
  return this._parseFileResponse(response.data);
      ...
      if (response.code === RESPONSE_CODES.INVALID_SESSION) {
        if (_retried) throw new Error('AniDB file lookup failed: session invalid after re-login');
        // Session expired, retry once (non-recursive)
        console.log('[AniDB UDP] Session invalid during FILE, re-logging in for one retry...');
        this.sessionKey = null;
        this.loggedIn = false;
        await this.loginToAniDB();
        return this.lookupFile(ed2kHash, fileSize, true);
      }
  ...
  /**
   * Lookup episode details by episode ID
   */
  async lookupEpisode(eid, _retried = false) {
    // Ensure socket is initialized before any operations
    this._initSocket();
    
    // Ensure we're logged in
    if (!this.sessionKey || !this.loggedIn || Date.now() >= this.sessionExpiry) {
      console.log('[AniDB UDP] Session invalid or expired for episode lookup, logging in...');
      await this.loginToAniDB();
    }
    
    console.log('[AniDB UDP] Looking up episode:', eid);
    
    try {
      const response = await this._sendCommand('EPISODE', {
        s: this.sessionKey,
        eid: eid
      }, false);
      
      if (response.code === '240') { // EPISODE response code
        return this._parseEpisodeResponse(response.data);
      }
      
      if (response.code === '340') { // NO SUCH EPISODE
        console.log('[AniDB UDP] Episode not found in AniDB');
        return null;
      }
      
      if (response.code === RESPONSE_CODES.INVALID_SESSION) {
        if (_retried) throw new Error('AniDB episode lookup failed: session invalid after re-login');
        // Session expired, retry once (non-recursive)
        console.log('[AniDB UDP] Session invalid during EPISODE, re-logging in for one retry...');
        this.sessionKey = null;
        this.loggedIn = false;
        await this.loginToAniDB();
        return this.lookupEpisode(eid, true);
      }
      
      throw new Error(`AniDB episode lookup failed: ${response.code}`);
      
    } catch (err) {
      console.error('[AniDB UDP] Episode lookup error:', err.message);
      throw err;
    }
  }

  /**
   * Look up anime by AID (anime ID)
   */
  async lookupAnime(aid, _retried = false) {
    // Ensure socket is initialized before any operations
    this._initSocket();
    
    // Ensure we're logged in
    if (!this.sessionKey || !this.loggedIn || Date.now() >= this.sessionExpiry) {
      console.log('[AniDB UDP] Session invalid or expired for anime lookup, logging in...');
      await this.loginToAniDB();
    }
    
    console.log('[AniDB UDP] Looking up anime:', aid);
    
    try {
      // Request basic anime info with comprehensive data
      // amask: request year, type, and all name variants (English, Romaji, Kanji, short, synonyms)
      // Using proven mask value that returns fields in expected order
      const response = await this._sendCommand('ANIME', {
        s: this.sessionKey,
        aid: aid,
        amask: 'B2F0E0FC' // Comprehensive anime data including all titles
      }, false);
      
      if (response.code === '230') { // ANIME response code
        return this._parseAnimeResponse(response.data);
      }
      
      if (response.code === '330') { // NO SUCH ANIME
        console.log('[AniDB UDP] Anime not found in AniDB');
        return null;
      }
      
      if (response.code === RESPONSE_CODES.INVALID_SESSION) {
        if (_retried) throw new Error('AniDB anime lookup failed: session invalid after re-login');
        // Session expired, retry once (non-recursive)
        console.log('[AniDB UDP] Session invalid during ANIME, re-logging in for one retry...');
        this.sessionKey = null;
        this.loggedIn = false;
        await this.loginToAniDB();
        return this.lookupAnime(aid, true);
      }
      
      throw new Error(`AniDB anime lookup failed: ${response.code}`);
      
    } catch (err) {
      console.error('[AniDB UDP] Anime lookup error:', err.message);
      throw err;
    }
  }
  ...
  /**
   * Parse EPISODE command response
   * Format: eid|aid|length|rating|votes|epno|eng|romaji|kanji|aired|type
   */
  _parseEpisodeResponse(data) {
    console.log('[AniDB Episode Parse] Raw data:', data.slice(0, 200));
    ...
    if (!normalized.length) {
      console.warn('[AniDB Episode Parse] Empty payload received');
      return { raw: data };
    }
    ...
    const fields = recordLine.split('|');
    console.log('[AniDB Episode Parse] Field count:', fields.length);

    const result = {
      raw: data,
      eid: fields[0] || null,
      aid: fields[1] || null,
      length: fields[2] || null,
      rating: fields[3] || null,
      votes: fields[4] || null,
      epno: fields[5] || null,
      englishName: fields[6] || null,
      romajiName: fields[7] || null,
      kanjiName: fields[8] || null,
      aired: fields[9] || null,
      type: fields[10] || null
    };

    console.log('[AniDB Episode Parse] Parsed:', {
      eid: result.eid,
      epno: result.epno,
      english: result.englishName || '<none>',
      romaji: result.romajiName || '<none>'
    });
  ...
  /**
   * Parse ANIME command response
   * Format: aid|year|type|romaji|kanji|english|...
   * (varies based on amask)
   */
  _parseAnimeResponse(data) {
    console.log('[AniDB Anime Parse] Raw data:', data.slice(0, 200));
    ...
    if (!normalized.length) {
      console.warn('[AniDB Anime Parse] Empty payload received');
      return { raw: data };
    }
    ...
    const fields = recordLine.split('|');
    console.log('[AniDB Anime Parse] Field count:', fields.length);
    console.log('[AniDB Anime Parse] Fields:', fields.map((f, i) => `[${i}]: ${f.slice(0, 50)}`));

    // Parse fields - order based on amask B2F0E0FC
    // Field 3 is categories/tags, titles start at field 4
    // aid|year|type|categories|romaji|kanji|english|synonyms|...
    const result = {
      raw: data,
      aid: fields[0] || null,
      year: fields[1] || null,
      type: fields[2] || null,
      categories: fields[3] || null,
      romajiName: fields[4] || null,
      kanjiName: fields[5] || null,
      englishName: fields[6] || null,
      shortNames: fields[7] || null,
      synonyms: fields[8] || null,
      // Also store raw fields for debugging
      _allFields: fields
    };

    console.log('[AniDB Anime Parse] Parsed:', {
      aid: result.aid,
      year: result.year,
      type: result.type,
      categories: result.categories?.slice(0, 30),
      'fields[4]': fields[4]?.slice(0, 30),
      'fields[5]': fields[5]?.slice(0, 30),
      'fields[6]': fields[6]?.slice(0, 30),
      english: result.englishName || '<none>',
      romaji: result.romajiName || '<none>',
      kanji: result.kanjiName || '<none>'
    });
