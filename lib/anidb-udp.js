...
class AniDBUDPClient {
  ...
  _parseFileResponse(data) {
    ...
    const groupFieldCount = (REQUESTS_GROUP_NAME ? 1 : 0) +
      (REQUESTS_GROUP_SHORT_NAME ? 1 : 0) +
      (REQUESTS_DATE_AID_RECORD ? 1 : 0);

    if (REQUESTS_EPISODE_KANJI && fields.length > groupFieldCount) {
