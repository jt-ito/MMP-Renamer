...
async function fetchSeriesById(creds, seriesId, log) {
  if (!creds || !seriesId) return null
  try {
    const token = await ensureTokenV4(creds, false, log)
    if (!token) return null
...
async function fetchMovieById(creds, movieId, log) {
  if (!creds || !movieId) return null
  if (log) {
    try { log(`TVDB_MOVIE_FETCH mode=v4 movieId=${movieId}`) } catch (e) {}
  }
  const path = `/v4/movies/${encodeURIComponent(movieId)}`
  const res = await apiRequest(path, creds, {}, log)
  if (!res || res.statusCode !== 200) {
    if (log) {
      try { log(`TVDB_MOVIE_FETCH_RESULT mode=v4 movieId=${movieId} status=${res ? res.statusCode : '<none>'}`) } catch (e) {}
    }
    return null
  }
  let parsed
  ...
  else if (data.released) { const m = String(data.released).match(/^(\d{4})/); if (m) year = m[1] }
  return { id: String(data.id), name: data.name || null, year, raw: Object.assign({}, data, { source: 'tvdb', media_type: 'movie' }) }
}

module.exports = { fetchEpisode, extractEpisodeTitle, fetchSeriesExtended, fetchSeriesById, fetchEpisodeBySeriesId, fetchEpisodeBySeries, fetchMovieById }
