...
async function fetchSeries(creds, name, log) {
  ...
  for (const item of list) {
    ...
    for (const tRaw of titles) {
      ...
      if (log) {
        try { log(`TVDB_SERIES_SCORE mode=v4 candidate="${safe(t)}" target="${safe(name)}" score=${score.toFixed(3)}`) } catch (e) {}
      }
...
function overlapScore(candidate, target) {
  ...
  // Penalize extra words: if candidate has more tokens than target, reduce score
  // This prevents "Restaurant to Another World in Singapore!" from scoring higher than "Restaurant to Another World"
  const lengthPenalty = Math.max(0, candidateTokens.length - targetTokens.length) * 0.1
