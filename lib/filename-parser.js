...
module.exports = function parseFilename(name) {
  ...
  try {
    ...
    const isPath = /[\\/]/.test(original);
    if (isPath) {
      const normalized = original.replace(/\//g, path.sep);
      const parent = path.basename(path.dirname(normalized));
      parentDirName = parent ? String(parent) : null;
    }
  } catch (e) { parentDirName = null }
  ...
  // We'll preserve decimal markers from the original basename so episode
  // detection continues to work even when we prefer the parent directory
  // for the series title. Use `basename` (the file's base name) for episode
  // detection, and optionally use `parentDirName` only to override the
  // final title when it looks like the computed title is actually the
  // episode text rather than the series title.
  ...
  const seasonMarker = withoutBrackets.match(/\bS0*(\d{1,2})(?:[EPp]0*(\d{1,3}(?:\.\d+)?))?(?=\b|[^a-z0-9]|$)/i);
    ...
    if (explicit) {
      ...
      episode = (rawE.indexOf('.') >= 0) ? parseFloat(rawE) : parseInt(rawE, 10);
      episodeLocked = true;
  ...
  // mark that this episode was explicitly detected from a dash-style pattern
  episodeLocked = true;
  ...
  const patterns = [ /\bS(\d{1,2})[EPp](\d{1,3}(?:\.\d+)?)\b/i, /\bS(\d{1,2})E(\d{1,3}(?:\.\d+)?)\b/i, /\b(\d{1,2})x(\d{1,3})\b/i, /\b(?:Ep(?:isode)?|E)\.?[ _-]?(\d{1,3})\b/i ];
  let match = null;
  ...
  if (match) {
    if (/^S\d+/i.test(match[0])) {
      ...
      const eVal = (rawE.indexOf('.') >= 0) ? parseFloat(rawE) : parseInt(rawE, 10);
    ...
    // mark that this was an explicit pattern match so enforcement keeps it
    episodeLocked = true;
  ...
  try { title = title.replace(/\b(19|20)\d{2}\b/g, '').replace(/\s+/g, ' ').trim(); } catch (e) { /* ignore */ }
  ...
  try {
    if (preferParentDir && parentDirName) {
      ...
      if (looksLikeEpisode) {
        title = cleanSeries(parentDirName || title);
        title = title.replace(/\bS0*\d{1,2}(?:[EPp]0*\d{1,3}(?:\.\d+)?)?\b/ig, ' ').replace(/\bSP\b/ig, ' ').replace(/\bP0*\d{1,3}\b/ig, ' ').replace(/\s+/g, ' ').trim();
      }
    }
  } catch (e) { /* ignore */ }
  // Enforce policy: do not infer episode numbers from filenames when they were
  // only heuristically detected in noisy release names. However, preserve
  // explicit SxxEyy/Pxx markers (episodeLocked) and fractional/season-0
  // episodes which are intentionally allowed.
  try {
    const hasEpisode = (typeof episode !== 'undefined' && episode !== null);
    const isDecimal = hasEpisode && (Number(episode) % 1 !== 0);
    // keepEpisode when it's an explicit marker (episodeLocked), a decimal episode,
    // or a season 0 special. Otherwise clear the inferred episode info.
    const keepEpisode = hasEpisode && (episodeLocked || isDecimal || (season === 0));
    if (!keepEpisode) {
      episode = null;
      episodeRange = null;
      season = null;
