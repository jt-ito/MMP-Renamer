...
module.exports = function parseFilename(name) {
  ...
  if (multiPartMatch) {
    ...
    title = cleanSeries(baseTit + ' Part ' + partNum);
  ...
  for (let i = 0; i < toks.length; i++) {
    if (i === 0) continue
    if (isNoiseToken(toks[i])) { toks.splice(i); break; }
  }
  while (toks.length > 0 && NOISE_TOKENS.has(toks[toks.length-1].toLowerCase())) toks.pop()
