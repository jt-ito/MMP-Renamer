...
module.exports = function parseFilename(name) {
  ...
  try {
    const seasonMarker = withoutBrackets.match(/\bS0*(\d{1,2})(?:[EPp]0*(\d{1,3}))?(?=\b|[^a-z0-9]|$)/i);
    if (seasonMarker) {
    ...
    try {
      if (anchorIdx >= 0 && (episode == null)) {
        const explicit = withoutBrackets.match(/\bS0*(\d{1,2})[EPp]0*(\d{1,3})\b/i);
        if (explicit) {
  ...
  const patterns = [ /\bS(\d{1,2})[EPp](\d{1,3})\b/i, /\bS(\d{1,2})E(\d{1,3})\b/i, /\b(\d{1,2})x(\d{1,3})\b/i, /\b(?:Ep(?:isode)?|E)\.?[ _-]?(\d{1,3})\b/i ];
  let match = null;
  ...
  try {
    ...
    const allE = Array.from(withoutBrackets.matchAll(/\bS0*(\d{1,2})E0*(\d{1,3})\b/ig));
    if (allE.length > 0) explicitFinal = allE[allE.length - 1];
    if (!explicitFinal) {
      const allP = Array.from(withoutBrackets.matchAll(/\bS0*(\d{1,2})[Pp]0*(\d{1,3})\b/ig));
      if (allP.length > 0) explicitFinal = allP[allP.length - 1];
    }
    if (explicitFinal) {
      season = parseInt(explicitFinal[1], 10);
      episode = parseInt(explicitFinal[2], 10);
