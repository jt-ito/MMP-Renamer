...
module.exports = function parseFilename(name) {
  ...
  const preservedDecimals = String(original).replace(/(S0*\d{1,2}[EPp]0*\d{1,3})\.(\d+(?:\.\d+)?)/ig, (m, g1, g2) => `${g1}::DEC::${g2}`);
    ...
    if (explicitFinal) {
      ...
      try {
        ...
        if (idx >= 0) {
          ...
          // If the explicit marker is at the beginning (left is empty), treat the right-hand
          // text as the series title (so titles like 'S01E11.5 The Poppies ...' become
          // 'The Poppies ...' instead of leaving the S01E.. token in the title).
          if (left) {
            title = left;
          } else if (right) {
            title = right;
            // we've consumed the right-hand text as the series title, so do not also
            // treat it as an episode title candidate below.
            right = '';
          }

          // remove dual-audio and normalize for episodeTitle extraction (only when left existed)
          if (left) {
            right = right.replace(/\b(?:audio[._\- ]*dual|dual[._\- ]*audio)\b/ig, ' ').replace(/\+/g, ' ').replace(/\s+/g, ' ').trim();
