...
module.exports = function parseFilename(name) {
  ...
  // Final cleanup: if the title still ends with a numeric token like " - 08",
  // treat that numeric as the episode number and remove it from the title. This
  // ensures filenames like "Chuhai Lips - 08 [WEB ...]" are parsed as title="Chuhai Lips" episode=8
  // (avoid 4-digit years).
  try {
    const trailingNumMatch = title.match(/^(.*?)[\s_\-–—]+0*(\d{1,3})$/);
    if (trailingNumMatch) {
      const left = trailingNumMatch[1].trim();
      const num = parseInt(trailingNumMatch[2], 10);
      if (!Number.isNaN(num) && String(num).length <= 3) {
        // only apply when this seems likely to be an episode (not a year)
        if (episode == null || episode === undefined) {
          title = cleanSeries(left) || title;
          episode = num;
          if (season == null) season = 1;
        } else {
          // if episode already detected but title still contains the numeric, strip it
          title = cleanSeries(left) || title;
        }
      }
    }
