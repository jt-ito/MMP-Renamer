...
module.exports = function parseFilename(name) {
  const original = String(name || '').trim().replace(/[‘’`´]/g, "'");
  
  // Custom Regex Override
  if (global.customRegexes && global.customRegexes.length > 0) {
    for (const regex of global.customRegexes) {
      try {
        const m = original.match(regex);
        if (m && m.groups) {
          const { title, season, episode, episodeRange, episodeTitle, year } = m.groups;
          if (title || episode != null || season != null) {
            let parsedName = title ? String(title).trim() : original;
            const parsedSeason = season != null ? parseInt(season, 10) : null;
            const parsedEpisode = episode != null ? parseEpisodeNumber(episode) : null;
            const parsedRange = episodeRange ? String(episodeRange).trim() : null;
            
            if (parsedRange) {
              const epLabel = (parsedSeason != null) ? `S${String(parsedSeason).padStart(2,'0')}E${parsedRange}` : `E${parsedRange}`;
              parsedName = `${parsedName} - ${epLabel}`;
            } else if (parsedEpisode != null) {
              const pad = n => String(n).padStart(2,'0');
              const epLabel = (parsedSeason != null) ? `S${pad(parsedSeason)}E${pad(parsedEpisode)}` : `E${pad(parsedEpisode)}`;
              parsedName = `${parsedName} - ${epLabel}`;
            }
            if (episodeTitle) {
              parsedName += ` - ${String(episodeTitle).trim()}`;
            }
            return {
              original,
              title: title ? String(title).trim() : original,
              season: parsedSeason,
              episode: parsedEpisode,
              episodeRange: parsedRange,
      ...
      } catch (e) { /* ignore individual regex error */ }
    }
  }
  ...
  try {
    if (parentDirName) {
      ...
      for (let i = 0; i < toks.length; i++) {
        ...
        // A 4-digit year (e.g. 2003) marks the boundary between title and release metadata;
        // stop here so edition tags that follow (EXT, REM, DC, etc.) don't leak into the title.
        if (RELEASE_NOISE.has(tl) || REGEX_RES_PATTERN.test(tl) || /^(19|20)\d{2}$/.test(tl)) { toks.splice(i); break; }
      }
  ...
  // also treat a 4-digit year as a cut point — everything after the year is release metadata.
  for (let i = 0; i < toks.length; i++) {
    if (i === 0) continue
    if (isNoiseToken(toks[i]) || /^(19|20)\d{2}$/.test(toks[i])) { 
      toks.splice(i); break; }
  }
