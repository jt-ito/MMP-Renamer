...
module.exports = function parseFilename(name) {
  ...
  // Enforce policy: do not infer episode numbers from filenames except for
  // season 0 special episodes or fractional episodes (e.g., 11.5).
  // This prevents noisy or incorrect episode detection from release names.
  try {
    const hasEpisode = (typeof episode !== 'undefined' && episode !== null)
    const isDecimal = hasEpisode && (Number(episode) % 1 !== 0)
    const keepEpisode = hasEpisode && (isDecimal || (season === 0))
    if (!keepEpisode) {
      episode = null
      episodeRange = null
      season = null
