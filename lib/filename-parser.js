...
function cleanToken(str) {
  ...
  // Preserve ellipsis (...) by temporarily replacing with a placeholder
...
module.exports = function parseFilename(name) {
  ...
  const specialKeywords = new Set(['sp', 'special', 'ova', 'oad', 'ona']);
  for (let i = toks.length - 1; i >= 0; i--) {
    const tok = toks[i].toLowerCase();
    if (specialKeywords.has(tok) && i + 1 <= toks.length - 1 && /^\d+$/.test(toks[i + 1])) {
