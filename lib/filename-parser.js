...
module.exports = function parseFilename(name) {
  ...
  const explicit = withoutBrackets.match(/\bS0*(\d{1,2})[EPp]0*(\d{1,3}(?:\.\d+)?)\b/i);
        if (explicit) {
            ...
            episode = parseInt(explicit[2], 10);
            episodeLocked = true;
  ...
  { re: /^(.+?)[\s_\-–—]+0*(\d{1,3}(?:\.\d+)?)[\s_\-–—]+(.+)$/, type: 'single' }
  ];
  ...
  const patterns = [ /\bS(\d{1,2})[EPp](\d{1,3}(?:\.\d+)?)\b/i, /\bS(\d{1,2})E(\d{1,3}(?:\.\d+)?)\b/i, /\b(\d{1,2})x(\d{1,3})\b/i, /\b(?:Ep(?:isode)?|E)\.?[ _-]?(\d{1,3}(?:\.\d+)?)\b/i ];
  let match = null;
  ...
  try {
    title = title.replace(/\bS0*\d{1,2}(?:[EPp]0*\d{1,3})?\b/ig, ' ').replace(/\bSP\b/ig, ' ').replace(/\bP0*\d{1,3}\b/ig, ' ').replace(/\s+/g, ' ').trim();
  } catch (e) { /* ignore */ }
  ...
  episode = parseFloat(explicitFinal[2]);
      ...
      try {
        ...
        if (idx >= 0) {
          ...
          // remove dual-audio and normalize
  ...
  // support fractional episodes (e.g., 11.5) by preserving decimal part
  const episodeLabelPart = (Number.isInteger(Number(episode))) ? `${pad(episode)}` : (() => { const parts = String(episode).split('.'); return `${pad(parts[0])}.${parts[1]}` })();
  const epLabel = (season != null) ? `S${pad(season)}E${episodeLabelPart}` : `E${episodeLabelPart}`;
    parsedName = `${title} - ${epLabel}`;
  }
