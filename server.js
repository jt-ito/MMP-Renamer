...
function appendLog(line) {
  try {
    ...
    if (logsFile && typeof logsFile === 'string') {
      ...
      // logsFile not available (e.g. test environment); fall back to console
...
// Atomic JSON writer: write to temp file then rename to avoid corruption on crash
function writeJson(filePath, obj) {
  try {
    const tmp = String(filePath) + '.tmp';
    fs.writeFileSync(tmp, JSON.stringify(obj, null, 2), { encoding: 'utf8' });
    try {
      fs.renameSync(tmp, filePath);
    } catch (renameErr) {
      // rename can fail across devices; fall back to direct write
...
const ctx = {
  ...
  bcrypt, cookieParser, cookieSession, titleCase, normalizeApostrophes,
  REGEX_NEWLINES, REGEX_MULTI_SPACE, REGEX_WHITESPACE, REGEX_APOSTROPHES,
  REGEX_QUOTES, REGEX_DOT_UNDER_DASH_COLON, REGEX_NON_ALPHANUM_SPACE,
  REGEX_NON_ALPHANUM, REGEX_NON_ALPHANUM_SPLIT, REGEX_SEASON_PARENS,
  REGEX_SEASON_PARENS_YEAR, REGEX_SEASON_SUFFIX, REGEX_ORDINAL_SEASON,
  REGEX_WORD_SEASON, REGEX_SEASON_WORD, REGEX_WORD_SEASON_YEAR,
  REGEX_SEASON_WORD_YEAR, REGEX_SXX_SUFFIX, REGEX_SXXEXX, REGEX_SXX,
  REGEX_EXX, REGEX_NXN, REGEX_EPISODE, REGEX_BRACKETS, REGEX_SEASON_KEYWORD,
  METADATA_PROVIDER_IDS, DEFAULT_METADATA_PROVIDER_ORDER,
  FOLDER_WATCH_RESTART_DELAY_MS, PROVIDER_DISPLAY_NAMES,
  DEFAULT_ALLOWED_ORIGINS, CSRF_COOKIE_NAME, CSRF_HEADER_NAME,
  SAFE_CSRF_METHODS, DATA_DIR, settingsFile, usersFile, enrichStoreFile,
  parsedCacheFile, scanStoreFile, scanCacheFile, renderedIndexFile,
  logsFile, manualIdsFile, approvedSeriesImagesFile, wikiEpisodeCacheFile,
  wikiSearchLogFile, SESSION_KEY, serverSettings, users, allRegexesInit,
  manualIds, envAllowedOrigins, configuredAllowedOrigins, allowedOrigins,
  allowedOriginsNormalized, baseCorsOptions, corsOptionsDelegate,
  ...
  loadManualIds, getManualId, isOriginTrusted, ensureCsrfToken,
  verifyCsrfToken, attachCsrfToken, requireAuth, requireAdmin, coerceBoolean,
  normalizeProviderId, appendLog, writeJson, persistEnrichCacheNow,
  schedulePersistEnrichCache, getEffectiveScanInputPath,
  isFolderWatchEnabledForUser, healCachedEnglishAndMovieFlags,
  startFolderWatcher, stopFolderWatcher, initializeAllWatchers,
  broadcastEvent, canonicalize, doProcessParsedItem, extractProviderRaw,
...
app.use(require('./routes/scan')(ctx));
app.use(require('./routes/enrich')(ctx));
app.use(require('./routes/rename')(ctx));
app.use(require('./routes/jobs')(ctx));
app.use(require('./routes/approved-series')(ctx));
app.use(require('./routes/debug')(ctx));
